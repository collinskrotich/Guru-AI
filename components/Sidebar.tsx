...
//         <button className={`w-full mt-12 mb-8 flex items-center justify-center bg-slate-300 hover:bg-slate-400 rounded-full py-2 ${isCollapsed ? 'px-2' : 'px-4'} transition-colors`}>
//           <Plus size={20} />
...
//               key={item.id}
//               href={item.link}
//               className={`flex items-center py-2 px-4 rounded-lg mb-2 ${activeMenuId === item.id ? 'bg-sky-200' : 'hover:bg-slate-200'} transition-colors`}
...
const Sidebar = () => {
  ...
  const handleLinkClick = (e, link) => {
    ...
    // Trigger your existing page change mechanism here
    // For example, if you're using a custom event system:
    // document.dispatchEvent(new CustomEvent('pageChange', { detail: { path: link } }));
  };
