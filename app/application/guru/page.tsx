...
const GuruCard: React.FC<GuruCardProps> = ({ title, description, link, Icon, iconColor, isSelected, onClick }) => (
  <Card 
    className={`bg-white hover:bg-gray-50 transition-all duration-300 cursor-pointer shadow-sm ${isSelected ? 'bg-blue-50 border-blue-200' : ''}`}
    onClick={onClick}
  >
    <CardContent className="p-6">
      <div className="flex items-center justify-between mb-4">
      ...
      <CardTitle className="text-xl font-bold mb-2 text-gray-900">{title}</CardTitle>
      <p className="text-gray-600">{description}</p>
    </CardContent>
  </Card>
);
...
const GURUManager: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-10">
      <h1 className="text-4xl font-bold mb-8 text-blue-900">AgentVerse</h1>
      
      <div className="mb-8">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-blue-800">Premade by Safaricom ML</h2>
          <Button
            variant="ghost"
            className="text-blue-600 hover:text-blue-700"
            onClick={() => setShowAll(!showAll)}
